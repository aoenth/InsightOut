...
struct Emoji: View {
    ...
    var body: some View {
        ZStack {
            Circle()
                .fill(Color.white)
                .frame(width: emojiInfo.frameSize, height: emojiInfo.frameSize)
            
            EmojiEyes(emojiInfo: emojiInfo)
                .offset(y: emojiInfo.frameSize * -0.1)
            EmojiMouth(emojiInfo: emojiInfo)
                .stroke(style: emojiInfo.selectedEmotion != Mood.surprised ? StrokeStyle(lineWidth: emojiInfo.frameSize * 0.08, lineCap: .round) : StrokeStyle(lineWidth: emojiInfo.frameSize * 0.1, lineCap: .round))
                .fill(emojiInfo.backgroundColor)
                .offset(y: emojiInfo.frameSize * 0.2)
                .frame(width: emojiInfo.frameSize * 0.5, height: emojiInfo.frameSize * 0.5)
        }
    }
}
...
struct EmojiEyes: View {
    ...
    var body: some View {
        HStack(spacing: emojiInfo.frameSize * 0.2) {
            ...
            case Mood.happiness:
                ForEach(0..<2) {_ in
                    Circle()
                        .fill(emojiInfo.backgroundColor)
                        .frame(width: emojiInfo.frameSize * eyeScale, height: emojiInfo.frameSize * eyeScale)
                }
            case Mood.sadness:
                ForEach(0..<2) {_ in
                    Circle()
            ...
            case Mood.love:
                ForEach(0..<2) {_ in
                    Image(systemName: "heart.fill")
                        ...
                        .font(Font.system(size: emojiInfo.frameSize * 0.2, weight: .semibold))
                        .frame(width: emojiInfo.frameSize * eyeScale, height: emojiInfo.frameSize * eyeScale)
                }
            case Mood.fear:
                ForEach(0..<2) {_ in
            ...
            case Mood.disgust:
                HStack(spacing: emojiInfo.frameSize * 0.16 ) {
                    EmojiDisgust()
                        .stroke(style: StrokeStyle(lineWidth: emojiInfo.frameSize * 0.07, lineCap: .round))
                        .rotation(Angle(degrees: 90))
                        .frame(width: emojiInfo.frameSize * 0.1, height: emojiInfo.frameSize * 0.3)
            ...
            case Mood.anger:
                HStack(spacing: emojiInfo.frameSize * 0.1) {
                ZStack {
                    Capsule()
                        .rotation(Angle(degrees: 20))
                        .fill(emojiInfo.backgroundColor)
                        
                        .frame(width: emojiInfo.frameSize * eyeScale * 1.5, height: emojiInfo.frameSize * eyeScale * 0.5)
                        .offset(y:emojiInfo.frameSize * -0.1 )
                    Circle()
                        .fill(emojiInfo.backgroundColor)
                        .frame(width: emojiInfo.frameSize * eyeScale, height: emojiInfo.frameSize * eyeScale)
...
struct EmojiMouth: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        Path { path in
            ...
            case Mood.surprised:
                let size = emojiInfo.frameSize * 0.2
                path.move(to: CGPoint(x: rect.midX, y: rect.midY))
...
struct EmojiIcon_Previews: PreviewProvider {
    ...
    static var previews: some View {
        EmojiIcon(emojiInfo: EmojiInfo(backgroundColor: Color.black, selectedEmotion: Mood.disgust  , frameSize: 200))
    }
}
